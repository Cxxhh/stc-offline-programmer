...
#include <stdbool.h>
    ...
    typedef void (*hal_error_callback_t)(stc_hal_status_enum error);
    ...
    {
        /* 基本通信接口 */
        stc_hal_status_enum (*fpinit)(void);                                      /**< 初始化函数 */
        void (*fpdeinit)(void);                                              /**< 反初始化函数 */
        stc_hal_status_enum (*fptransmit)(const uint8_t *data, uint16_t length); /**< 发送数据 */
        stc_hal_status_enum (*fpreceive)(uint8_t *data, uint16_t length);        /**< 接收数据 */
        stc_hal_status_enum (*fptransmit_receive)(const uint8_t *tx_data, uint8_t *rx_data, uint16_t length); /**< 收发数据 */
        ...
        /* 波特率配置 - 串口通信必需 */
        stc_hal_status_enum (*fpset_baudrate)(uint32_t baudrate);               /**< 设置波特率 */
    ...
    #define STC_HAL_TIMEOUT_CHECK(start_tick, timeout_ms, get_tick_func) \
